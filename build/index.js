...
(tokenizer.match(/require\('?"?[^\']*'?"?\)/ig) || []).forEach(function(_require){
	var _path = _require.replace(/require\('?"?/, '').replace(/'?"?\)/, '');
	var inludeFile = fs.readFileSync(path.resolve(__dirname, '../source', _path), 'utf-8').replace(reME, '').replace(/\;[\b\n\t\r ]*$/, '');
	
	tokenizer = tokenizer.replace(_require, inludeFile);
});
